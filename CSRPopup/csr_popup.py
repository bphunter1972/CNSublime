...
def parse_csr(flines, reg_dict):
    ...
    reg = re.compile(r"(?P<longname>(^\$\S+))(?P<desc>(.*?)(?=^$))",
...
def find_reg(reg, wks_path, csr_list, blk_dict, reg_dict):
    ...
    try:
        proc = subprocess.Popen('/bin/grep "class ' + reg_class + '" '
                                + csr_gen_files,
                                shell=True,
        ...
        return("<p>Unexpected error while building register name " +
               reg + " in " + wks_path + "</p>")

    sout = out.decode("utf-8")
    # print("Grep ###" + sout + "###")
    if sout == "":
        return("<p>Register name " + reg +
               " not found in any csr gen files in " +
               wks_path + "</p>")
    ...
    for line in lines:
        ...
        if item:
            ...
            if found is False:
                ...
                if csr_file is None:
                    desc = desc + "<p>Block name " + blk + \
                           " not found in csr_list for register name " + \
                           reg + "</p>"
                else:
                    # print("blk " + blk)
                    # print("csr_file " + csr_file)
                    blk_dict[blk] = True
                    desc = desc + find_reg_in_csr(reg, csr_file, reg_dict)
                    desc = desc + "<p></p>"
...
class CavmCsrPopupCommand(sublime_plugin.TextCommand):
    ...
    def run(self, edit):
        ...
        if self.wks_path == "":
            sublime.error_message("Cannot find workset path from current file"
                                  + self.view.file_name())
            return

        if len(self.csr_list) == 0:
            sublime.status_message("First time csr_list initialization...")
            self.csr_list = find_file_pattern('*.csr.txt',
                                              self.wks_path + '/rtl')
        ...
        if msg:
            ...
            s = '<style> p {color: black; font-family: monospace;} </style>'
            s = s + msg
